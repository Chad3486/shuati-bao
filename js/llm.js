...
const LLM = (() => {
  ...
  /* ---- 单次 chat 调用 ---- */
  async function chat(messages, { onRetry, raw = false, signal = null, onDelta = null, modelOverride = null, _solveMode = false } = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetry; attempt++) {
      ...
      try {
        ...
        const timeout = _solveMode ? 30000 : (useStream ? 60000 : 90000);
        let timer = null;
        timer = setTimeout(() => ctl.abort(Object.assign(new Error('请求超时（网络连不通）'), { timeout: true })), timeout);
        if (_solveMode) console.log('[解题] 发送请求到:', url, 'model:', payload.model);
        ...
        if (!resp.ok) {
          if (_solveMode) console.log('[解题] API 响应错误:', resp.status);
          if (resp.status === 429) {
        ...
        let content;
        if (useStream) {
          if (attempt > 0 && onDelta) onDelta('');
          content = await readStream(resp, onDelta, ctl);
          if (!content) throw new Error('API 返回为空');
        } else {
          const data = await resp.json();
          if (_solveMode) console.log('[解题] API 返回数据:', JSON.stringify(data).slice(0, 200));
          content = data.choices?.[0]?.message?.content;
          if (!content) {
            if (_solveMode) console.log('[解题] content 为空，data:', JSON.stringify(data).slice(0, 300));
            throw new Error('API 返回为空');
          }
          if (data.usage) _lastUsage = data.usage;
        }
        if (_solveMode) console.log('[解题] 最终 content:', String(content).slice(0, 200));
        return content;
      } catch (e) {
        lastErr = e;
