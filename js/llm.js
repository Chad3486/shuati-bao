...
const LLM = (() => {
  ...
  const LEVELS = [
    { batch: 5, concurrency: 2 },   // 强网：5题/批，避免 GLM-Flash 等慢模型生成 10 题超 90s
    { batch: 3, concurrency: 2 },
...
- explanation 一句话即可，不要写小作文`;
    ...
    async function attemptBatch(batch) {
      ...
      const result = await chat([
        ...
        onRetry: (a, c) => {
          // chat 重试时立刻反馈到 UI，避免"转圈无报错"误判为卡死
          if (onProgress) {
            const note = c > 0 ? `限流冷却 ${c}s · 第 ${a} 次重试` : `网络抖动 · 第 ${a} 次重试`;
            onProgress(processed, total, solved, note);
          }
        },
        maxTokens,
        withFinish: true
      ...
      // 鲁棒提取答案数组：兼容 GLM/不同模型可能返回的多种 JSON 结构
      // 期望: {answers:[{idx,answer,explanation}]} 或 [{idx,answer,...}] 或 {answers:{"0":{...}}}
      let arr = null;
      if (Array.isArray(obj)) {
        arr = obj;                                            // 直接数组
      } else if (obj && typeof obj === 'object') {
        arr = obj.answers;                                     // 期望格式
        if (!Array.isArray(arr)) {
          // answers 可能是 {"0":{...}, "1":{...}} 这种对象映射
          if (arr && typeof arr === 'object') {
            arr = Object.values(arr);
          } else if (Array.isArray(obj.questions)) {          // 模型返回 questions
            arr = obj.questions;
          } else if (Array.isArray(obj.data)) {               // 模型返回 data
            arr = obj.data;
          } else if (Array.isArray(obj.results)) {            // 模型返回 results
            arr = obj.results;
          } else if (Array.isArray(obj.list)) {
            arr = obj.list;
          } else if (obj.idx !== undefined || obj.answer !== undefined) {
            // 单个对象 {idx,answer,...} 当成只有一题
            arr = [obj];
          } else {
            // 最后兜底：扫所有 value 找第一个数组
            arr = Object.values(obj).find(v => Array.isArray(v)) || null;
          }
        }
      }
      if (Array.isArray(arr)) {
        for (const a of arr) {
          ...
          const q = batch[a.idx] || batch[a.index] || batch[a.i];
          if (!q || q.answer) continue;
          let ans = String(a.answer ?? a.ans ?? a.A ?? '').trim();
          if (!ans) continue;
          if (q.options) {
      ...
      // 无效 JSON 或一题都没解出 → 失败，把模型实际返回的内容前 80 字反馈到 UI 帮助诊断
      if (!Array.isArray(arr) || solvedHere === 0) {
        if (onProgress) {
          const preview = (result.content || '(空)').replace(/\s+/g, ' ').slice(0, 80);
          onProgress(processed, total, solved, `解析失败 · GLM 返回: ${preview}`);
        }
    ...
    async function worker() {
      while (queue.length) {
        ...
        // 启动一批前先反馈，避免 fetch pending 期间 UI 不动 → "转圈无报错"误判为卡死
        if (onProgress) {
          onProgress(processed, total, solved, `正在解 ${batch.length} 题 · 档位 ${_netLevel + 1}/4`);
        }
...
- explanation 写清推理依据，2-3 句即可，不要长篇大论`;
    ...
    async function attemptBatch(batch) {
      ...
      const result = await chat([
        ...
        onRetry: (a, c) => {
          if (onProgress) {
            const note = c > 0 ? `限流冷却 ${c}s · 第 ${a} 次重试` : `网络抖动 · 第 ${a} 次重试`;
            onProgress(processed, total, checked, note);
          }
        },
        maxTokens,
        withFinish: true
      ...
      if (Array.isArray(arr)) {
        for (const a of arr) {
          ...
          let ai = String(a.answer ?? a.ans ?? a.A ?? '').trim();
          if (!ai) continue;
          if (q.options) {
      ...
      if (!Array.isArray(arr) || checkedHere === 0) {
        if (onProgress) {
          const preview = (result.content || '(空)').replace(/\s+/g, ' ').slice(0, 80);
          onProgress(processed, total, checked, `解析失败 · GLM 返回: ${preview}`);
        }
        return { ok: false, truncated: false };
      }
    ...
    async function worker() {
      while (queue.length) {
        ...
        // 启动一批前先反馈，避免 fetch pending 期间 UI 不动 → "转圈无报错"误判为卡死
        if (onProgress) {
          onProgress(processed, total, checked, `正在校验 ${batch.length} 题 · 档位 ${_netLevel + 1}/4`);
        }
