    ...
    aiFileInput.onchange = async () => {
      ...
      try {
        const text = await aiCanonFromFiles(files, {
          ...
          onProgress: p => { aiBar.style.width = Math.round(p * 100) + '%'; BG.update(`AI 转范式：完成 ${Math.round(p * 100)}%`); },
    ...
    if (toCanonBtn) toCanonBtn.onclick = async () => {
      ...
      try {
        const text = await aiCanonFromFiles(lastPickedFiles, {
          ...
          onProgress: p => { bar.style.width = Math.round(p * 100) + '%'; BG.update(`AI 转范式：完成 ${Math.round(p * 100)}%`); }
        });
  ...
  async function pageAnswers(params) {
    ...
    if (noAns.length) {
      ...
      aiBtn.onclick = async () => {
        ...
        // 锁屏保活：批量解题可能几十分钟，前台服务保证不被冻结（每批进度同步到通知栏）
        BG.start(`AI 解答中 · 共 ${todo.length} 题`);
        ...
        try {
          const ret = await LLM.solveMissing(noAns,
            ({ done, total, solved, usage, cost, paused, stopReason, note }) => {
              ...
              BG.update(`AI 解题：已解出 ${solved} 题 · 进度 ${done}/${total} 批`);
              // 每批解出的立即落库，断网/退出不丢
          ...
          aiProg.style.display = 'none';
          BG.stop(); // 任务结束（完成/暂停/中断）都撤通知、释放唤醒锁
        }
      };
    }
  }
